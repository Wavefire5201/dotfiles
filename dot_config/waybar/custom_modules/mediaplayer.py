...
class PlayerManager:
    ...
    def on_metadata_changed(self, player, metadata, _=None):
        ...
        # Skip if no title (likely nothing playing)
        if not title:
            self.clear_output()
            return
        ...
        if track_info:
            if player.props.status == "Playing":
                ...
                track_info = " " + track_info
                # Don't show anything when paused/stopped
                if not self.get_first_playing_player():
                    self.clear_output()
                    return
        ...
        if (
            ...
            if current_playing is None and player.props.status != "Playing":
                # Don't display anything when nothing is actively playing
                self.clear_output()
            else:
